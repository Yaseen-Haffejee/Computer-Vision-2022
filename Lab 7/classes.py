...
class Piece:
	...
	def update_edges(self, transform):
		# TO DO: Update the corner and edge information of the puzzle piece
		newCorners = [] #update corner info
		for i in self.corners:
			newCorners.append(np.array([i[1],i[0],1]).dot(transform.T))

		newCornersFlipped = []
		for i in newCorners: #flip back after updating
			newCornersFlipped.append([i[1],i[0]])
		self.corners = np.array(newCornersFlipped)

		for i in self.edge_list: #update edge info
			if(i != None):
				temp = np.array([i.point1[1],i.point1[0],1]).dot(transform.T)
				i.point1 = np.array([temp[1],temp[0]])
				temp = np.array([i.point2[1],i.point2[0],1]).dot(transform.T)
				i.point2 = np.array([temp[1],temp[0]])
		return
	...
	def insert(self,canvas): # Inserts the piece into the canvas using an affine transformation
		...
		pts_dst = []
		for edge in self.edge_list:
			if(edge != None and edge.connected_edge != None):
				if(edge.connected_edge.parent_piece.inserted == True):
					count = count + 1
		if(count == 0): #inserting corner piece
			self.piece_type = "corner"
			if(self.top_edge.is_flat != None and self.left_edge.is_flat != None and (self.top_edge.point2 == self.left_edge.point1).all()):
				first_edge = self.top_edge
				second_edge = self.left_edge
			elif(self.left_edge.is_flat != None and self.bottom_edge.is_flat != None and (self.left_edge.point2 == self.bottom_edge.point1).all()):
				first_edge = self.left_edge
				second_edge = self.bottom_edge
			elif(self.bottom_edge.is_flat != None and self.right_edge.is_flat != None and (self.bottom_edge.point2 == self.right_edge.point1).all()):
				first_edge = self.bottom_edge
				second_edge = self.right_edge
			else:
				first_edge = self.right_edge
				second_edge = self.top_edge
			#bottom left
			pts_src.append(np.flip(first_edge.point2))
			pts_dst.append([0,800])

			#lie LHS of canvas
			pts_src.append(np.flip(first_edge.point1))
			newPoint = [0,800-((first_edge.point1[0]-first_edge.point2[0])**2+(first_edge.point1[1]-first_edge.point2[1])**2)**(1/2)]
			pts_dst.append(newPoint)

			#lie bottom of canvas
			pts_src.append(np.flip(second_edge.point2))
			newPoint = [0+((second_edge.point1[0]-second_edge.point2[0])**2+(second_edge.point1[1]-second_edge.point2[1])**2)**(1/2),800]
			pts_dst.append(newPoint)
			...
			self.update_edges(M)
			for i in range(3):
				canvas[:,:,i] = self.mask*self.dst[:,:,i] + (1-self.mask)*canvas[:,:,i]		
		elif(count == 1): #inserting edge piece
			self.piece_type = "edge"
			for j in range(len(self.edge_list)-1):
				edge = self.edge_list[j]
				if(edge != None and edge.connected_edge != None and edge.connected_edge.parent_piece.inserted == True):
					third_edge = edge
					pts_src.append(np.flip(third_edge.point2))
					pts_src.append(np.flip(third_edge.point1))
					pts_dst.append(np.flip(third_edge.connected_edge.point1))
					pts_dst.append(np.flip(third_edge.connected_edge.point2))
					orig_norm = ((third_edge.point1[0]-third_edge.point2[0])**2+(third_edge.point1[1]-third_edge.point2[1])**2)**(1/2)
					canvas_norm = ((third_edge.connected_edge.point1[0]-third_edge.connected_edge.point2[0])**2+(third_edge.connected_edge.point1[1]-third_edge.connected_edge.point2[1])**2)**(1/2)
					ratio = orig_norm/canvas_norm
					if(pts_dst[0][0]-pts_dst[1][0]) < (pts_dst[0][1]-pts_dst[1][1]):
						if(j == 0):
							fourth_edge = self.edge_list[3]
						else:
							fourth_edge = self.edge_list[j-1]

						pts_src.append(np.flip(fourth_edge.point1))
						edge_norm = ((fourth_edge.point1[0]-fourth_edge.point2[0])**2+(fourth_edge.point1[1]-fourth_edge.point2[1])**2)**(1/2)
						pts_dst.append(np.array([pts_dst[1][0]+int(ratio*edge_norm),pts_dst[1][1]]))
					else:
						if(j == 0):
							fifth_edge = self.edge_list[3]
						else:
							fifth_edge = self.edge_list[j-1]
						pts_src.append(np.flip(fifth_edge.point1))
						edge_norm = ((fifth_edge.point1[0]-fifth_edge.point2[0])**2+(fifth_edge.point1[1]-fifth_edge.point2[1])**2)**(1/2)
						pts_dst.append(np.array([pts_dst[1][0],pts_dst[1][1]-int(ratio*edge_norm)]))
					M = cv2.getAffineTransform(np.float32(pts_src),np.float32(pts_dst))
					self.dst = cv2.warpAffine(self.image, M,(700,800))
					self.mask = cv2.warpAffine(self.mask, M,(700,800))
					self.update_edges(M)
		...
		elif(count == 2): #inserting interior piece
			self.piece_type = "interior"
			for edge in self.edge_list:
				if(edge != None and edge.connected_edge != None and edge.connected_edge.parent_piece.inserted == True):
					checkIn = False
					for i in pts_src:
						if((edge.point1 == np.flip(i)).all()):
							checkIn = True
							break
					if(checkIn == False):
						pts_src.append(np.flip(edge.point1))
						pts_dst.append(np.flip(edge.connected_edge.point2))
					checkIn = False
					for i in pts_src:
						if((edge.point2 == np.flip(i)).all()):
							checkIn = True
							break
					if(checkIn == False):
						pts_src.append(np.flip(edge.point2))
						pts_dst.append(np.flip(edge.connected_edge.point1))
			M = cv2.getAffineTransform(np.float32(pts_src),np.float32(pts_dst))
			self.dst = cv2.warpAffine(self.image, M,(700,800))
			self.mask = cv2.warpAffine(self.mask, M,(700,800))
			self.update_edges(M)
			...
			raise Exception("Sorry, no count above 2")
